...
fn main() {
    ...
    let mut file_nova_pp = File::create("../../public/serialized_outputs/uncompressed_IVCProof_only/nova_pp_output.bin").unwrap();
        file_nova_pp.write_all(&nova_pp_serialized).unwrap();
        println!("nova_pp written to nova_pp_output.bin");
    let mut file_nova_vp = File::create("../../public/serialized_outputs/uncompressed_IVCProof_only/nova_vp_output.bin").unwrap();
